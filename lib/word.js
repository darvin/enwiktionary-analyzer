...
Explanation = function Explanation(opts) {
  ...
  if (opts.mentionedWords && opts.mentionedWords.length>0)
    this.mentionedWords = opts.mentionedWords;
};
...
Object.defineProperty(Explanation.prototype, "toFragmentsAndWordLinks", {
  value: function() {
    var fragments = this.explanation.split(WORDLINK_PLACEHOLDER);
    var result = [];
    for (var i=0; i<fragments.length; i++) {
      result.push(fragments[i]);
      if (this.mentionedWords && this.mentionedWords[i])
        result.push(this.mentionedWords[i]);
    }
