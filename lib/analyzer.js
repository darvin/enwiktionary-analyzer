...
var ParsoidConfig = require('../node_modules/parsoid/lib/mediawiki.ParsoidConfig.js').ParsoidConfig;
var parseJs = require('../node_modules/parsoid/tests/parse.js');
...
function parsoidParse(wikitext, callback) {
	...
	var prefix = "enwiki";
	var parsoidConfig = new ParsoidConfig(ls, { defaultWiki: prefix });
  parsoidConfig.addHTMLTemplateParameters = true;
	parseJs.parse(wikitext, argv, parsoidConfig, prefix).then(function(res) {
		callback(null,  new Parsoid.PDoc(res.env, res.out) );
	}).done();
}
